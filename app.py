...
# --- Получение постов с пагинацией (стабильная версия) ---
def get_posts(category, page=1, per_page=6):
    try:
        ...
        total = int(content_range.split('/')[-1]) if '/' in content_range and content_range.split('/')[-1].isdigit() else 0

        # Если header не дал total — fallback
        if total == 0:
