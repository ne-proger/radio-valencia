...
# --- Получение постов с пагинацией (без headers, стабильный count) ---
def get_posts(category, page=1, per_page=6):
    try:
        ...
        # Общее количество постов в категории (без Prefer — просто len)
        all_posts_resp = read_client.from_('post').select("id").eq('category', category).execute()
        total = len(all_posts_resp.data)
