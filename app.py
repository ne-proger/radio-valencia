...
def get_posts(category, page=1, per_page=6):
    ...
    # Общее количество постов в категории (Prefer: count=exact)
    count_resp = read_client.from_('post').select("id").eq('category', category).execute(headers={"Prefer": "count=exact"})
    content_range = count_resp.headers.get('Content-Range', '0-0/0')
    total = int(content_range.split('/')[-1]) if '/' in content_range else len(pinned)

    # Обычные посты с range (учитываем pinned)
    offset = (page - 1) * per_page
    limit = per_page
    normal_resp = read_client.from_('post').select("*").eq('category', category).eq('is_pinned', False).order('date_posted', desc=True).range(offset, offset + limit - 1).execute()
